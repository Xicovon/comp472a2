...
class HeuristicA:
    ...
    def start(self):
        ...
        while current_node.grid != self.goal_state and not no_solution:
            ...
            if not skip:
                # The current Node has not been expanded yet
        ...
        print("Heuristic A elapsed time: {}".format(elapsed_time))
        ...
        if no_solution:
            f = open(self.file_path + "heuristic_a_solution_path_{}.txt".format(self.puzzle_number), "w")
            f.write("no_solution")
            f.close()

            f = open(self.file_path + "heuristic_a_search_path_{}.txt".format(self.puzzle_number), "w")
            f.write("no_solution")
            f.close()
        else:
            path.reverse()
            self.write_to_file(path, self.file_path + "heuristic_a_solution_path_{}.txt".format(self.puzzle_number))

            # output searched states
            self.write_to_file(visited_nodes, self.file_path + "heuristic_a_search_path_{}.txt".format(self.puzzle_number))
